...
function userAccConnected(){getMyStakes()
getSellRequests()
mainContract.methods.soldStakeFunds(user.address).call({shouldPollResponse:true}).then(res=>{$('.tmn-12')[0].innerHTML=`You have - ${parseInt(res)/1e18} BNB - collectable from your sold stakes! click to collect.`})}
function refreshMyStakes(){$('.stake-ls')[0].innerHTML=""
getMyStakes()}
function sellStakeFinal(){if(!parseFloat($('.sell-price')[0].value))return
$('.mt-no')[0].innerHTML="..."
let val=parseFloat($('.sell-price')[0].value)*1000000
mainContract.methods.sellStakeRequest(currentTargetStakeId,(BigInt(val)*BigInt(1e18))/BigInt(1000000)).send({from:user.address,shouldPollResponse:true}).then(res=>{console.log(res)
doAlert(`Successfully created sell offer for ${parseFloat($('.sell-price')[0].value)} BNB`,3)
closeModal()
$('.mt-no')[0].innerHTML="Sell"}).catch(err=>{doAlert("Something went wrong!",2)
closeModal()
console.log(err)}).finally(res=>{closeModal()
$('.mt-no')[0].innerHTML="Sell"})}
function getMyStakes(){setavrgPrice()
mainContract.methods.calcStakeCount(user.address).call({shouldPollResponse:true}).then(res=>{if(parseInt(res)>0){getMyStakes_det(parseInt(res))
...
if(stake.stake_forSell){if(stake.price.length<25)actBtn=actionButton_sell_alreadyforsell+actionButton_sell_alreadyforsell_cancel}
...
function cancelSellOffer(id){mainContract.methods.sellStakeRequest(id,web3.utils.toWei('1000000','ether')).send({from:user.address,shouldPollResponse:true}).then(res=>{doAlert(`Successfully cancelled sell offer.`,3)
refreshClick
...
function getSellRequests(){if(!moralisRecentEvents||!moralisRecentEvents[3]){setTimeout(()=>{getSellRequests()},500)}else{beforeRender_checkTimeStamp(moralisRecentEvents[3])
...
if(BigInt(parseInt(el.price))<900000n*1000000000000000000n){if(parseInt(el.timestamp)>(Date.now()/1000)-expireAtDayCount)newData.push(el)}})
console.log(newData)
...
if(alreadyProccesesOffer==false){mainContract.methods.mapMemberStake(address,stakeId).call({shouldPollResponse:true}).then(res=>{res.timestamp=timestamp
if(res.price.length<25){stakesToBeRendered.push(res)
runRenderIn()}})
...
if(itm.stake_forSell==true&&parseInt(itm.endDay)>currentDay&&itm.tokenValue.length>14){let progress=parseInt(Math.abs((((itm.endDay-currentDay)/(itm.endDay-itm.startDay))*100)-100))
...
$('.grid-container-fs')[0].innerHTML+=`
        ...
        `
if((itm.userAddress).toLocaleUpperCase()===(user.address).toLocaleUpperCase()){$(`.v-i-77-${ide}`)[0].innerHTML=`Selling For ${(parseInt(itm.price)/1e18).toFixed(4)} BNB`
$(`.v-i-77-${ide}`)[0].style.background="linear-gradient(to bottom right, #cddc39 0%, #4caf50 100%)"}
getStakeDivs(itm.stakeId,ide)
function getStakeDivs(id,ide){mainContract.methods.calcStakeCollecting(itm.userAddress,id).call({shouldPollResponse:true}).then(res=>{$(`.v-i-5-${ide}`)[0].innerHTML=(parseInt(res)/1e18).toFixed(3)+" BNB"})}}})}
...
getMyStakes()}else{$('.grid-container-fs')[0].innerHTML=""
getSellRequests()}}
let buyingStake={address:void 0,id:void 0,price:void 0}
function buyAStake(sellerAddress,stakeId){openModal2()
mainContract.methods.mapMemberStake(sellerAddress,stakeId).call({shouldPollResponse:true}).then(res=>{$('.mod-txt')[0].innerHTML="You are buying A stake for "+(parseInt(res.price)/1e18).toFixed(5)+" BNB"
buyingStake.price=res.price
buyingStake.address=sellerAddress
buyingStake.stakeId=stakeId})}
function buyAStake_B(){mainContract.methods.buyStakeRequest(buyingStake.address,buyingStake.stakeId).send({from:user.address,shouldPollResponse:true,value:parseInt(buyingStake.price)}).then(res=>{console.log(res)
...
refreshClick(1)
refreshClick(2)})}
function collectStakeSoldReward(){mainContract.methods.withdrawSoldStakeFunds().send({from:user.address,shouldPollResponse:true}).then(res=>{doAlert(`Successfully Collected!`,3)}).catch(err=>{doAlert("Something went wrong!",2)
console.log(err)})}
function refreshGlobalData(){mainContract.methods.totalTradeAmount().call({shouldPollResponse:true,}).then(res=>{$('.gd-31')[0].innerHTML=(parseInt(res)/1e18).toFixed(2)})
mainContract.methods.totalStakesSold().call({shouldPollResponse:true,}).then(res=>{$('.gd-32')[0].innerHTML=parseInt(res)})
mainContract.methods.totalStakeTradeAmount(user.address).call({shouldPollResponse:true,}).then(res=>{$('.user-data-3')[0].value=(parseInt(res)/1e18).toFixed(2)})
...
function setAvgPriceFinal(){$('.price-avr-txt')[0].innerHTML=`Average AVC price [past 3 days]: ${avgPrice.toFixed(5)} BNB`}}