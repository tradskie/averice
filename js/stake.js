...
$('.do-stake-btn-txt')[0].innerHTML="Stake"
console.log(err)}).finally(res=>{$('.do-stake-btn-txt')[0].innerHTML="Stake"})}}
function endStake(stake_id){console.log(stake_id,"675")
$('.bt-cl')[0].innerHTML="..."
mainContract.methods.EndStake(stake_id).send({from:user.address,shouldPollResponse:false}).then(res=>{doAlert(`Successfully collected the stake.`,3)
refreshMyStakes()}).catch(err=>{doAlert("Something went wrong!",2)
$('.bt-cl')[0].innerHTML="Collect"
console.log(err)}).finally(res=>{$('.bt-cl')[0].innerHTML="Collect"})}
let onlyUnfinished=false
function userAccConnected(){onlyUnfinished=false
getMyStakes()}
function refreshClick(){$('.stake-ls')[0].innerHTML=""
getMyStakes()}
function hideFinished(){onlyUnfinished=true
$('.stake-ls')[0].innerHTML=""
getMyStakes()}
function getMyStakes(){totaldata.staked=0
totaldata.divs=0
totaldata.bonus=0
...
if($('.loading-tx-1')[0])$('.loading-tx-1')[0].remove()}else if(parseInt(res)==0){$('.loading-tx-1')[0].innerHTML="You have no stakes!"}})
getLobbyData()}
function getMyStakes_det(count){let curentId=0
let triedIdLast=0
getOneStake(curentId)
function getOneStake(id){mainContract.methods.mapMemberStake(user.address,id).call({shouldPollResponse:true}).then(res=>{if(onlyUnfinished){if(res.stake_hasSold==false&&parseInt(res.endDay)>currentDay)renderAStake(res,id)}else{if(res.stake_hasSold==false)renderAStake(res,id)}
triedIdLast++
if(triedIdLast<count){getOneStake(triedIdLast)}else if(triedIdLast==count){setTimeout(()=>{doTotalRow()},1000)}})}}
...
function doTotalRow(){let bgRow="background: #f3f2f7"
...
let totaldata={staked:0,divs:0,bonus:0}
...
if(stake.startDay-1==currentDay){progress="starting soon"}else
if((stake.startDay-1==currentDay)||stake.endDay>currentDay){progress=Math.abs((((stake.endDay-currentDay)/(stake.endDay-stake.startDay))*100)-100)
...
progress=`<div style="
            ...
            "></div></div>`}else{progress="completed"
...
getStakeDivs(id)
function getStakeDivs(id){mainContract.methods.calcStakeCollecting(user.address,id).call({shouldPollResponse:true}).then(res=>{$(`.ltv-5-${id}`)[0].innerHTML=(parseInt(res)/1e18).toFixed(3)
...
function refreshGlobalData(){mainContract.methods.overall_stakedTokens().call({shouldPollResponse:true,}).then(res=>{$('.gd-31')[0].innerHTML=((parseInt(res)/1e18)-412330).toFixed(2)})
mainContract.methods.overall_collectedDivs().call({shouldPollResponse:true,}).then(res=>{$('.gd-32')[0].innerHTML=(parseInt(res)/1e18).toFixed(2)})
mainContract.methods.overall_collectedBonusTokens().call({shouldPollResponse:true,}).then(res=>{$('.gd-33')[0].innerHTML=(parseInt(res)/1e18).toFixed(2)})
...
web3.eth.getBalance("0xaef3f73a20d19b998e85e6fcdb89cf4279be2853",function(err,result){if(err){console.log(err)}else{$('.uns-13')[0].innerHTML=(web3.utils.fromWei(result)*94/100).toFixed(2)+" BNB"}})}
