...
$('.auction-ls')[0].innerHTML+=`<tr role="row" class="" style="${bgRow}">
        ...
        <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
            <div data-v-254b7dbb="" class="d-flex align-items-center">
                <div data-v-254b7dbb="" class="ltv-333-${i} ltv-33 font-small-2 text-muted">${lobbyPoolclc(i+1)}</div>
            </div>
        </td>
        <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
            <div data-v-254b7dbb="" class="d-flex align-items-center">
...
function setActionCollectButton(_i){mainContract.methods.mapMemberLobby(user.address,_i+1).call({shouldPollResponse:true}).then(res=>{if(res[2]==true){$(`.ltv-7-${_i}`)[0].innerHTML=actionButton_collect_inactive}else if(res[2]==false){$(`.ltv-7-${_i}`)[0].innerHTML=`
...
$(`.ltv-5-${_day-1}`)[0].innerHTML=(parseFloat($(`.ltv-333-${_day-1}`)[0].innerHTML)/parseFloat(totalDayEntery)).toFixed(2)
requestCounter()}
